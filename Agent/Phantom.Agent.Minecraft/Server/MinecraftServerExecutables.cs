...
public sealed class MinecraftServerExecutables {
	...
	private readonly Dictionary<string, Task<string?>> runningDownloadTasksByVersion = new ();
	...
	public async Task<string?> DownloadAndGetPath(string version, CancellationToken cancellationToken) {
		...
		lock (this) {
			if (runningDownloadTasksByVersion.TryGetValue(version, out var existingTask)) {
				Logger.Information("A download for server version {Version} is already running, waiting for it to finish...", version);
				task = existingTask;
			}
			else {
				task = DownloadAndGetPathImpl(version, serverExecutableFilePath, cancellationToken);
				runningDownloadTasksByVersion[version] = task;
			}
		}
		...
		lock (this) {
			runningDownloadTasksByVersion.Remove(version);
		}

		return result;
	}

	private async Task<string?> DownloadAndGetPathImpl(string version, string filePath, CancellationToken cancellationToken) {
		...
		try {
			...
			Logger.Information("Downloading server executable from: {Url}", serverExecutableInfo.DownloadUrl);
			try {
	...
	private static ServerExecutableInfo GetServerExecutableUrlFromMetadata(JsonElement versionMetadata) {
		...
		long size = sizeElement.GetInt64();

		if (size <= 0L) {
			Logger.Error("The \"size\" key in downloads.server object in version metadata contains an invalid file size: {Size}", size);
			throw StopProcedureException.Instance;
		}
		...
		return new ServerExecutableInfo(url, hash, size);
	}
